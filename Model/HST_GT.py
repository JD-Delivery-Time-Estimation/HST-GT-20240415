...
class HGT(torch.nn.Module):
    def __init__(self, hidden_channels, num_heads, num_layers,temporal_features,back_features,data):
        ...
        self.skip_gru_transformation = torch.nn.ModuleDict()
        for node_type in data.node_types:
            self.skip_gru_transformation[node_type] = Linear(4*hidden_channels,2*hidden_channels)

    def forward(self, x_dict, edge_index_dict,train_temporal_data,train_back_data,mask_in,mask_pack_in,if_first,cmask,dmask,t):
        out = {}
        ...
        if if_first:
            ...
            for i in gru_init:
                gru_out1[i],gru_hid[i] = self.grus[i](gru_in[i],gru_init[i])
                self.skip_gru_hidden[i][t] = gru_hid[i]
            if t < 24:
                for i in gru_out1:
                    gru_out2[i] = gru_out1[i]
                    gru_out3[i] = gru_out1[i]
            if t >= 24 and t < 7 *24:
                for i in gru_out1:
                    gru_out3[i] = gru_out1[i]
                gru_out2[i],gru_hid[i] = self.grus[i](gru_in[i],self.skip_gru_hidden[i][t-24])
            if t >= 7 * 24:
                gru_out2[i],gru_hid[i] = self.grus[i](gru_in[i],self.skip_gru_hidden[i][t-24])
                gru_out3[i],gru_hid[i] = self.grus[i](gru_in[i],self.skip_gru_hidden[i][t-7*24])
            ...
            for i in gru_in:
                gru_out1[i],gru_hid[i] = self.grus[i](gru_in[i])
                self.skip_gru_hidden[i][t] = gru_hid[i]
            if t < 24:
                for i in gru_out1:
                    gru_out2[i] = gru_out1[i]
        ...
        for i in gru_out1:
            gru_out1[i] = gru_out1[i].view(-1,self.hidden_size)
        for i in gru_out2:
            gru_out2[i] = gru_out1[i].view(-1,self.hidden_size)
        for i in gru_out3:
            gru_out3[i] = gru_out1[i].view(-1,self.hidden_size)
        ...
        for i in gru_out1:
            xt[i] = (gru_out1[i],gru_out2[i],gru_out3[i],back[i])
            xt[i] = F.relu(self.skip_gru_transformation[i](torch.cat(xt[i],dim=1)))
